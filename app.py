...
# Constants
GRID_SIZE = 8  # Define an 8x8 grid
DATA_FILE = "cued_click_points.json"  # File to save registered points
TOLERANCE = 5  # Pixel tolerance thingi
...
class CuedClickPointsApp:
    def __init__(self, root):
        ...
        self.max_points = 3  # Number of points to register/login
    ...
    def load_image(self):
        ...
        if not self.image_path:
            messagebox.showerror("Error", "No image loaded. Please upload an image.")
            return
        try:
    ...
    def register_mode(self):
        ...
        # Ask the user whether to upload an image or use a random one
        choice = messagebox.askyesno("Image Choice", "Do you want to upload an image? (Yes: Upload, No: Use Random)")
        if choice:
            self.upload_image()
        else:
    ...
    def get_random_image(self):
        ...
        folder = "images"  # Change this to your image folder path
    ...
    def verify_login(self):
        ...
        if self.username in self.data:
            saved_points = self.data[self.username]["points"]
            if self.points_match(saved_points, self.selected_points):
                messagebox.showinfo("Success", "Login successful!")
            else:
                messagebox.showerror("Error", "Invalid points. Login failed.")
        else:
            messagebox.showerror("Error", "User data not found. Login failed.")
        self.reset()

    def points_match(self, saved_points, selected_points):
        """Check if the selected points match the saved points within a tolerance."""
        for (saved_row, saved_col), (selected_row, selected_col) in zip(saved_points, selected_points):
            if abs(saved_row - selected_row) > TOLERANCE or abs(saved_col - selected_col) > TOLERANCE:
                return False
        return True
